...
public class CudaRuntimeCompiler{
  ...
  public long nvrtcCreateProgram(String src, String name, String[] headers, String[] includeNames) throws Throwable{
    ...
    var prog = allocator.allocate(ValueLayout.ADDRESS); // nvrtcProgram is a pointer
    var cSrc = allocator.allocateUtf8String(src);
    Addressable cName = name == null ? MemoryAddress.NULL : allocator.allocateUtf8String(name);
    ...
    if(numHeaders > 0){
      cHeaders = allocator.allocateArray(ValueLayout.ADDRESS, numHeaders);
      cIncludeNames = allocator.allocateArray(ValueLayout.ADDRESS, numHeaders);
      for(int i = 0; i < numHeaders; i++){
        ((MemorySegment)cHeaders).setAtIndex(ValueLayout.ADDRESS, i, allocator.allocateUtf8String(headers[i]));
        ((MemorySegment)cIncludeNames).setAtIndex(ValueLayout.ADDRESS, i, allocator.allocateUtf8String(includeNames[i]));
      }
    }
  ...
  public long nvrtcGetPTXSize(long prog) throws Throwable{
    ...
    var allocator = SegmentAllocator.implicitAllocator();
    var cSize = allocator.allocate(ValueLayout.JAVA_LONG); // size_t
  ...
  public void nvrtcDestroyProgram(long prog) throws Throwable{
    ...
    var allocator = SegmentAllocator.implicitAllocator();
    var cProg = allocator.allocate(ValueLayout.JAVA_LONG, prog);
